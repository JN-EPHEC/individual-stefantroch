...
describe('Pair Whise userValidator', () => {
    test.each(ValidateUserTest)('Pour chaque situation : age %d, role %s, email %s', (age, role, email, expected) => {
        const result = userValidator(age, role, email);
        expect(result).toBe(expected);
    });
});
describe('Pair whise userValidator throw errors', () => {
    test.each(ValidateUserTestThrow)('Pour chaque situation : age %d, role %s, email %s', (age, role, email, expected) => {
